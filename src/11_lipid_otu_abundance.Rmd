---
title: "11_lipid_otu_abudance"
---

purpose: to average the TOTAL FAT (TFAT) amounts over 2 or 3 recalls as per CBS suggestion 06/03/2021
...
otu.total <- aggregate(TFAT ~ UserName + FoodCode , data=recalls.use,FUN="sum") #13339 x 3 can't use Food Description because that hasn't been uniquified yet (i.e. includes variations like W/OIL. That gets cleaned up in the next part)
...
otu.freq$avg <- otu.freq$TFAT/otu.freq$freq
```

```{r}
otu.fat<- otu.freq %>% select(UserName, FoodCode, avg) #this is the average protein values for all foods and subj passing qc (i.e. including water and the 7 subjects mary asked to remove)
colnames(otu.fat)<-c("UserName", "FoodCode", "FoodAmt") #the 'avg' column needs to be named "FoodAmt" for the below function
```

```{r}
#sanity check-- how many unique foods are in here? 
length(unique(recalls.use$FoodCode)) #2129
length(unique(otu.fat$FoodCode)) #2125

'%nin%'<- Negate('%in%')
which<- recalls.use %>% filter(FoodCode %nin% unique(otu.fat$FoodCode)) %>% select(FoodCode, Food_Description) %>% unique()
...
subj5002<- recalls.use %>% dplyr::filter(UserName == 5002) #consumed 42401010 coconut milk 3 times (28.608, 14.304, 42.912 g = 85.824g total)

freq5002<- otu.freq %>% filter(UserName == 5002)#foodcode 42401010 average is 85.824/3 = 28.608 correct

subj5012<- recalls.use %>% dplyr::filter(UserName == 5012) #consumed Milk, NFS 11100000 1x in recall 2, TFAT == 0.2379; has 3 recalls; consumed 53220030_Cookie, fig bar 4x, total TFAT = 7.008
freq5012<- otu.freq %>% filter(UserName ==5012) #foodcode 11100000, avg TFAT = 0.0793; foodcode 53220030, avg TFAT = 2.336 CORRECT
```
...
otu.fat$FoodCode_tmp<- as.integer(format(otu.fat$FoodCode, scientific = F))
for(i in 1:nrow(otu.fat)){
  if(otu.fat[i,"FoodCode_tmp"] == "94000000"){
    otu.fat[i,"FoodCode"] <- as.integer(94000000)
  }else if(otu.fat[i,"FoodCode_tmp"] == "20000000"){
    otu.fat[i,"FoodCode"] <- as.integer(20000000)
  }
}
...
```{r}
library(stringr)
water<- otu.fat %>% filter(str_detect(as.integer(FoodCode), regex("^94"))) #415 entries
nowater<- otu.fat %>% filter(!str_detect(as.integer(FoodCode), regex("^94"))) #12924 subjects, correct (13339-415), 2122 unique foodcodes
```
...
```{r}
write.table(filt, file = "../data/09_lipid_otu/avg_fat_g.txt", sep="\t", row.names=F, quote=F)
```
...
make.food.otu(food_records_fn = "../data/09_lipid_otu/avg_fat_g.txt" ,
              ...
              output_fn="../data/09_lipid_otu/fl100_fat_otu_tmp.txt")
...
```{r}
otu<- read.delim("../data/09_lipid_otu/fl100_fat_otu_tmp.txt", header=T, sep = "\t", stringsAsFactors = F)
...
write.table(tmp, "../data/09_lipid_otu/fl100_fat_otu.txt", row.names = F, sep = "\t", quote=F)
```
