---
title: "10_protein_otu_abudance"
---

purpose: to average the PROTEIN amounts over 2 or 3 recalls as per CBS suggestion 06/03/2021
...
otu.total <- aggregate(PROT ~ UserName + FoodCode , data=recalls.use,FUN="sum") #13339 x 3 can't use Food Description because that hasn't been uniquified yet (i.e. includes variations like W/OIL. That gets cleaned up in the next part)
...
otu.freq$avg <- otu.freq$PROT/otu.freq$freq
```

sanity checks
```{r}
subj5001<- recalls.use %>% dplyr::filter(UserName == 5001) #yes, consumed 94000100 15 times 
#ate 42116000 Walnuts, 1x in recall 2, PROT = 4.8736,subject 5001 has 3 reccalls

freq5001<- otu.freq %>% filter(UserName == 5001)#foodcode 42116000 average is 4.8736/3 = 1.624533 correct

subj5012<- recalls.use %>% dplyr::filter(UserName == 5012) #consumed Milk, NFS 11100000 1x in recall 2, PROT == 0.39894; has 3 recalls; consumed 53220030_Cookie, fig bar 4x, total PROT = 1.184
freq5012<- otu.freq %>% filter(UserName ==5012) #foodcode 11100000, avg PROT = 0.13298; foodcode 53220030, avg PROT = 1.184 CORRECT
```


```{r}
otu.prot<- otu.freq %>% select(UserName, FoodCode, avg) #this is the average protein values for all foods and subj passing qc (i.e. including water and the 7 subjects mary asked to remove)
colnames(otu.prot)<-c("UserName", "FoodCode", "FoodAmt") #the 'avg' column needs to be named "FoodAmt" for the below function
```

```{r}
#sanity check-- how many unique foods are in here? 
length(unique(recalls.use$FoodCode)) #2129
length(unique(otu.prot$FoodCode)) #2125

'%nin%'<- Negate('%in%')
which<- recalls.use %>% filter(FoodCode %nin% unique(otu.prot$FoodCode)) %>% select(FoodCode, Food_Description) %>% unique()
...
otu.prot$FoodCode_tmp<- as.integer(format(otu.prot$FoodCode, scientific = F))
for(i in 1:nrow(otu.prot)){
  if(otu.prot[i,"FoodCode_tmp"] == "94000000"){
    otu.prot[i,"FoodCode"] <- as.integer(94000000)
  }else if(otu.prot[i,"FoodCode_tmp"] == "20000000"){
    otu.prot[i,"FoodCode"] <- as.integer(20000000)
  }
}
...
```{r}
library(stringr)
water<- otu.prot %>% filter(str_detect(as.integer(FoodCode), regex("^94"))) #415 subjects
nowater<- otu.prot %>% filter(!str_detect(as.integer(FoodCode), regex("^94"))) #12924 subjects, correct, 2122 unique foodcodes
```
...
This is the average g of protein with water foods (FoodCode ^94 removed) and the 7 subjects removed
```{r}
#write.table(filt, file = "../data/08_protein_otu/avg_prot_g.txt", sep="\t", row.names=F, quote=F)
```
...
make.food.otu(food_records_fn = "../data/08_protein_otu/avg_prot_g.txt" ,
              ...
              output_fn="../data/08_protein_otu/fl100_protein_otu_tmp.txt")
...
```{r}
otu<- read.delim("../data/08_protein_otu/fl100_protein_otu_tmp.txt", header=T, sep = "\t", stringsAsFactors = F)
...
write.table(tmp, "../data/08_protein_otu/fl100_protein_otu.txt", row.names = F, sep = "\t", quote=F)
```
...
let's check that the taxonomy (last column) is the same as what was in the fiber OTU table: 

```{r}
fiber_otu<- read.table("../data/07_fiber_otu/fl100_fiber_otu.txt", sep = "\t", header=T)
dim(fiber_otu) #2107 345, same as above

#check that the columns are the same
ifelse(colnames(fiber_otu) == colnames(tmp), 1, 0) #all the same

#check that the taxonomy are the same
tiny_fiber<-fiber_otu %>% dplyr::select(X.FoodCode, taxonomy)
tiny_protein<- tmp %>% dplyr::select(X.FoodCode, taxonomy)
tiny_protein$X.FoodCode<- as.factor(tiny_protein$X.FoodCode)#convert to factor because tiny_fiber is factor
tiny_protein$taxonomy<-as.factor(tiny_protein$taxonomy)

all_equal(tiny_fiber, tiny_protein) #yes everything is the same!
```
